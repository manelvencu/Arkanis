...
function createNpcAnimations(scene: AldeaRuntime): void {
  (['boy', 'girl'] as NpcKind[]).forEach((kind) => {
    ...
    create('side', [
      `aldea-npc-${kind}-side1`,
      `aldea-npc-${kind}-side`,
      `aldea-npc-${kind}-side2`,
      `aldea-npc-${kind}-side`
    ], 8);
  });
}
...
function setNpcIdle(npc: VillageNpc): void {
  ...
  const texture = `aldea-npc-${npc.kind}-${npc.facing === 'side' ? 'side' : npc.facing}`;
...
function createVillageNpcs(scene: AldeaRuntime): void {
  ...
  scene.__villageNpcs = starts.map(({ kind, cabinIndex, extraDelay }) => {
    ...
    return {
      ...
      route: [],
      routeIndex: 0,
      waitingUntil: now + MIN_INSIDE_MS + extraDelay,
...
function updateVillageNpcs(scene: AldeaRuntime, time: number, delta: number): void {
  const npcs = scene.__villageNpcs ?? [];
